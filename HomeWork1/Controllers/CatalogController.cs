...
namespace HomeWork1.Controllers
{
    public class CatalogController : Controller
    {
        ...
        public async Task<IActionResult> Comics(int? pageNumber)
        {
            if (pageNumber < 0) pageNumber = 0;
            int pageSize = 1;
            int pageNums = pageNumber == null ? 1 : 0;
            var item = await _catalogArt.GetComics();
            
            var result = await PaginationList<UniversalCatalog>.CreateAsync(item, pageNumber ?? 1, pageSize);
            result.NextPage = (result.PageIndex == result.TotalPages - 1) ? "is-disabled" : string.Empty;
            result.PreviousPage = (result.PageIndex == 0) ? "is-disabled" : string.Empty;
